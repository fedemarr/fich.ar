...
function EstadoBadge({ estado }: { estado: ColaboradorEquipo["estado"] }) {
  ...
  if (estado === "novedad")
    return <Badge className="bg-blue-50 text-blue-700 border-blue-200 gap-1 text-xs"><FileText size={11} /> Novedad</Badge>
  if (estado === "ausente")
    return <Badge className="bg-red-50 text-red-700 border-red-200 gap-1 text-xs"><XCircle size={11} /> Ausente</Badge>
  return <Badge className="bg-gray-100 text-gray-500 border-gray-200 gap-1 text-xs"><Clock size={11} /> Sin registro</Badge>
}
...
export function EquipoCliente() {
  ...
  return (
    <div className="space-y-4 px-0">
      {/* Header */}
      <div className="flex items-start justify-between gap-3 px-4 pt-2 sm:px-0 sm:pt-0">
        <div>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 sm:px-0">
        <div className="bg-white border border-gray-200 rounded-xl p-3 text-center shadow-sm">
          <p className="text-2xl sm:text-3xl font-bold text-green-600">{presentes}</p>
          <p className="text-[11px] text-gray-500 mt-0.5">Presentes</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-3 text-center shadow-sm">
          <p className="text-2xl sm:text-3xl font-bold text-red-500">{ausentes}</p>
          <p className="text-[11px] text-gray-500 mt-0.5">Ausentes</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-3 text-center shadow-sm">
          <p className="text-2xl sm:text-3xl font-bold text-blue-600">{novedades}</p>
          <p className="text-[11px] text-gray-500 mt-0.5">Con novedad</p>
        </div>
      </div>

      {loading ? (
        <div className="text-center py-12 text-gray-400 text-sm">Cargando equipo...</div>
      ) : colaboradores.length === 0 ? (
        <div className="text-center py-12 text-gray-400 text-sm px-4">
          No hay colaboradores asignados a tus puntos
        </div>
      ) : (
        <>
          {/* Mobile: cards */}
          <div className="sm:hidden space-y-2 px-4">
            {colaboradores.map((c) => (
              <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-3.5 shadow-sm">
                ...
                {c.novedad && (
                  <p className="text-[11px] text-blue-600 mt-1.5 bg-blue-50 rounded-lg px-2 py-1">
                  ...
                  <div className="flex gap-4 mt-2 pt-2 border-t border-gray-100">
                    {c.entrada && (
                      <div>
                        ...
                        <p className={`text-sm font-semibold ${c.analisisEntrada === "LLEGADA_TARDE" ? "text-orange-600" : "text-gray-800"}`}>
                          {c.entrada}
                          {c.analisisEntrada === "LLEGADA_TARDE" && (
                            <span className="ml-1 text-[10px] font-normal text-orange-400">tarde</span>
                          )}
                        </p>
                      </div>
                    )}
                    {c.salida && (
                      <div>
                        ...
                        <p className="text-sm font-semibold text-gray-800">{c.salida}</p>
                      </div>
                    )}
                  </div>
                )}
